...
namespace ofxCv {
	...
	void FlowFarneback::setStepSize(int stepSize){
	...
	Mat FlowFarneback::getFlowAsAlpha(){
		cv::Mat xy[2];
		cv::split(abs(flow),xy);

		cv::Mat retval(flow.rows, flow.cols, CV_8UC1);
		xy[0].convertTo(xy[1], CV_8UC1, 255.0);
		return xy[1];
	}
	...
	void FlowFarneback::drawFlow(ofRectangle rect){
		...
		for(int y = 0; y < flow.rows; y += stepSize) {
			for(int x = 0; x < flow.cols; x += stepSize) {
				glm::vec2 flow_pos = getFlowPosition(x, y) * scale;
				glm::vec2 cur = glm::vec2(x, y) * scale;
				cur += (cur - flow_pos) * globalScale;
				ofDrawLine(cur, flow_pos);

				// add offset after calculating diffs
			}
		}
	}

	void FlowFarneback::drawCircles(const glm::vec2 & offset, const glm::vec2 & dims){
		if(!hasFlow){
			return;
		}

		glm::vec2 scale(dims.x/flow.cols, dims.y/flow.rows);
		...
		for(int y = 0; y < flow.rows; y += stepSize) {
			for(int x = 0; x < flow.cols; x += stepSize) {
				...
				glm::vec2 cur = glm::vec2(x, y) * scale;
				cur += (cur - flow_pos);
				float size = glm::distance(cur, getFlowPosition(x, y) * scale)*globalScale;
				ofDrawRectangle(cur, size, size);
	...
	template <class T>
	void FlowFarneback::getAsPixels(ofPixels_<T>& pixels){
		toOf(flow, pixels);
	}
}
